...
import com.xtkj.libmyapp.util.dct.FDCT;
...
public class AbImageUtil {
     ...
     * @return Bitmap 新图片
     */
    public static Bitmap getScaleBitmap(Context context,Uri uri, int desiredWidth, int desiredHeight) {
     ...
     * @return Bitmap 新图片
     */
    public static Bitmap getScaleInCenterBitmap(File file, int desiredWidth, int desiredHeight) {
        ...
        // 缩放的比例
        float scale = getMaxScale(srcWidth, srcHeight, desiredWidth, desiredHeight);
        int destWidth = srcWidth;
        int destHeight = srcHeight;
        if (scale != 0) {
            destWidth = (int) (srcWidth * scale);
     ...
     * @return Bitmap 新图片
     */
    public static Bitmap getScaleInCenterBitmap(Context context,Uri uri, int desiredWidth, int desiredHeight) {
     ...
     * @return Bitmap 新图片
     */
    public static Bitmap getScaleInCenterBitmap(Bitmap bitmap, int desiredWidth, int desiredHeight) {
     ...
     * @return Bitmap 新图片
     */
    public static Bitmap getScaleByOneSideBitmap(File file, int desiredWidth, int desiredHeight) {
     ...
     * @return Bitmap 新图片
     */
    public static Bitmap getScaleByOneSideBitmap(Bitmap bitmap, int desiredWidth, int desiredHeight) {
    ...
    /**
     * 获取缩小的比例.
     * 这个方法，是用来保证缩放后图片大小，是足够填充整个目标区域大小的
     * 也就是说，这样缩放后，是需要剪裁的
     * 放大的情况：
     * 取scale大的值，保证放大后，是足够填充的
     * 缩小的情况:
     * 取scale大的值，(scale<1)的，这时候，越大就表示缩小的越少。保证区域足够填充
     *
     * @param srcWidth
     * @param srcHeight
     * @param desiredWidth
    ...
    /**
     * 获取缩小的比例
     * 这个方法，是用来保证缩放后的图片，是可以完整的放置于整个目标区域，边缘不一定是填充满的
     * 相当于ScaleType.CenterInside
     *
     * @param srcWidth
     * @param srcHeight
     * @param desiredWidth
     ...
     * @return
     */
    private static float getMaxScale(int srcWidth, int srcHeight, int desiredWidth,
                                     int desiredHeight) {
        // 缩放的比例
        float scale = 0;
        // 计算缩放比例，宽高的最小比例
